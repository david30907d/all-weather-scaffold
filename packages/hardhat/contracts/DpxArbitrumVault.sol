...
contract DpxArbitrumVault is ERC4626 {
    ...
    ERC20 public immutable dpxToken = ERC20(0x6C2C06790b3E3E3c38e12Ee22F8183b37a13EE55);
    ERC20 public immutable sushiToken = ERC20(0xd4d42F0b6DEF4CE0383636770eF773390d85c61A);
    address public immutable oneInchAggregatorAddress = 0x1111111254fb6c44bAC0beD2854e76F90643097d;
    address public immutable sushiSwapRouterAddress = 0x1b02dA8Cb0d097eB8D57A175b88c7D8b47997506;
    address public immutable sushiSwapDpxLpTokenAddress = 0x0C1Cf6883efA1B496B01f654E247B9b419873054;

    ICloneRewarderTime public immutable dpxRewarder = ICloneRewarderTime(0xb873813F710093CBC17836297A6feFCfc6989faF);
    IMiniChefV2 public sushiSwapMiniChef;

    uint256 public pid; // sushiSwap pid
    constructor(IERC20Metadata asset_, address sushiSwapMiniChefV2_, uint256 pid_) 
        ERC4626(asset_)
        ERC20("DpxVault", "DPXV") 
    {
        pid = pid_;
    ...
    function deposit(uint256 amount, address receiver, bytes calldata oneInchData) public returns (uint256) {
    ...
    function redeemAll(uint256 shares, address receiver) public returns (uint256) {
    ...
    function _zapIn(uint256 amount, bytes calldata oneInchData) internal returns (uint256) {
        SafeERC20.safeApprove(IERC20(asset()), oneInchAggregatorAddress, Math.mulDiv(amount, 1, 2));
        (bool succ, bytes memory data) = address(oneInchAggregatorAddress).call(oneInchData);
        ...
        (uint amountToken, uint amountETH, uint liquidity) = IUniswapV2Router01(sushiSwapRouterAddress).addLiquidityETH{value: address(this).balance}(
            address(dpxToken),
            dpxReturnedAmount,
            ...
            address(this),
            deadline);
        IERC20(sushiSwapDpxLpTokenAddress).approve(address(sushiSwapMiniChef), liquidity);
