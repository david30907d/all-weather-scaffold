...
const oneInchJson = require('../artifacts/contracts/1inch/AggregationRouterV4.sol/AggregationRouterV4.json');
...
describe("All Weather Protocol", function () {
  beforeEach(async () => {
    ...
    dpxVault = await DpxArbitrumVault.deploy(weth.address, sushiMiniChefV2Address, sushiPid);
...
async function fetch1InchSwapData(fromTokenAddress, toTOkenAddress, amount, fromAddress) {
    ...
    // await (await fetch1InchSwapData()).json()).tx.data;
    // const iface = new ethers.utils.Interface(oneInchJson.abi);
    // const functionData = iface.decodeFunctionData('swap', resJson.tx.data);
