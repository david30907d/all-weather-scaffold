...
describe("All Weather Protocol", function () {
    ...
    describe("Portfolio LP Contract Test", function () {
        it("Should be able to zapin with WETH into equilibria PENDLE", async function () {
            ...
            for (const event of receipt.events) {
                if (event.topics.includes(equilibriaPendleVault.interface.getEventTopic('Deposit')) && event.address === equilibriaPendleVault.address) {
                    ...
                    if (decodedEvent.owner === portfolioContract.address) {
                        expect(await equilibriaPendleVault.balanceOf(portfolioContract.address)).to.equal(decodedEvent.shares);
                        expect((await equilibriaPendleVault.totalAssets())).to.equal(decodedEvent.shares);
                        expect(await portfolioContract.balanceOf(wallet.address)).to.equal(portfolioShares);
                        const [a, b, rewardpool, c] = await pendleBooster.poolInfo(4);
                        expect(await ethers.getContractAt("IERC20", rewardpool).then(async (contract) => await contract.balanceOf(equilibriaPendleVault.address))).to.equal(decodedEvent.shares);
                    }
                }
            }
        });
        it("Should be able to withdraw PENDLE from equilibria", async function () {
            this.timeout(240000); // Set timeout to 120 seconds
            const receipt = await deposit(end2endTestingAmount, wallet, pendleGLPZapInData, pendleGDAIZapInData, oneInchSwapDataForGDAI, oneInchSwapDataForRETH, pendleRETHZapInData, oneInchSwapDataForMagic, pendlePendleZapInData);
        ...
        it("Should be able to claim rewards", async function () {
            ...
            for (const claimableReward of claimableRewards) {
                if (claimableReward.protocol !== "Equilibria-PENDLE") {
            ...
            for (const remainingClaimableReward of remainingClaimableRewards) {
                if (remainingClaimableReward.protocol === "Equilibria-PENDLE") {
                    expect(remainingClaimableReward.claimableRewards[0].amount).to.equal(0);
                }
            }
            const xEqbTokenBalance = await xEqbToken.balanceOf(equilibriaPendleVault.address);
            await equilibriaPendleVault.connect(wallet).redeemXEQB(xEqbTokenBalance, 86400 * 7 * 24);
            timeElasped = 24 * 7 * 86400; // 24 weeks later
            await simulateTimeElasped(timeElasped);
            await equilibriaPendleVault.connect(wallet).finalizeRedeem(0);
